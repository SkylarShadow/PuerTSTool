...
export default abstract class UIBase {
  protected m_Event: EventDispatcher | null = null;
  ...
  public get Event(): EventDispatcher | null {
    return this.m_Event;
  }
   ...
   */
  AddToPlayerScreen(_zOrder?: number /* = 0 */): boolean {
    console.error("函数重载失效！");
    return false;
  }
  /*
   ...
   *@param ZOrder The higher the number, the more on top this widget will be.
   */
  AddToViewport(_zOrder?: number /* = 0 */): void {
    console.error("函数重载失效！");
  }
   ...
   */
  SetVisibility(_inVisibility: UE.ESlateVisibility): void {
    console.error("函数重载失效！");
  }
}
