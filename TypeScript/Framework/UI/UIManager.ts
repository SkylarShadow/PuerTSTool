  ...
  implements ISingleton<UIManager>
{
  ...
  private OnUIDestroy(widget: UE.UserWidget): void {
    if (widget) {
      const nIndex = this.m_arrCacheUI.indexOf(widget);
      if (nIndex != -1) {
   ...
   * @returns UI实例
   */
  public GetUI<T extends UE.UserWidget>(
    strUIName: string,
    bFuzzy: boolean = true,
  ): T | undefined {
  ...
  public GetOrCreateUI<T extends UE.UserWidget>(
    strUIPath: string,
    strUIName: string,
    bFuzzy: boolean = true,
  ): T {
  ...
  public RemoveUI(strUIName: string, bFuzzy: boolean = true): boolean {
    const widget = this.GetUI(strUIName, bFuzzy);
    if (widget) {
