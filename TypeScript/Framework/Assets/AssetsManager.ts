...
export default class AssetsManager implements ISingleton<AssetsManager> {
  ...
  Destroy(): void {
    for (const value of this.m_mapBPClass.values()) {
      Misc.GetTSSubsys().RemoveClass(value);
    }
    this.m_mapBPClass.clear();

    for (const value of this.m_mapBPObject.values()) {
  ...
  public LoadBPClass(strPath: string): UE.Class {
    const cachedClass = this.m_mapBPClass.get(strPath);
    if (cachedClass && Misc.IsValid(cachedClass)) {
      return cachedClass;
    }
    const bpClass = UE.Class.Load(strPath);
   ...
   * @memberof AssetsManager
   */
  public LoadBPClassAsync(
    strPath: string,
    callback: (bpClass: UE.Class) => void,
  ): void {
    const cachedClass = this.m_mapBPClass.get(strPath);
    if (cachedClass && Misc.IsValid(cachedClass)) {
      callback(cachedClass);
      return;
    }
  ...
  private async LoadBPClassAsyncImpl(
    strPath: string,
    callback: (bpClass: UE.Class) => void,
  ): Promise<void> {
    const bpClass = await Misc.AsyncLoad(strPath);
    this.CacheBPClass(strPath, bpClass);
    callback(bpClass);
  }
  ...
  public LoadBPObject(strPath: string): UE.Object {
    const cachedObject = this.m_mapBPObject.get(strPath);
    if (cachedObject && Misc.IsValid(cachedObject)) {
      return cachedObject;
    }
    const bpObject = UE.Object.Load(strPath);
   ...
   * @memberof AssetsManager
   */
  public LoadBPObjectAsync(
    strPath: string,
    callback: (bpObject: UE.Object) => void,
  ): void {
    const cachedObject = this.m_mapBPObject.get(strPath);
    if (cachedObject && Misc.IsValid(cachedObject)) {
  ...
  private async LoadBPObjectAsyncImpl(
    strPath: string,
    callback: (bpObject: UE.Object) => void,
  ): Promise<void> {
    const bpObject = await Misc.AsyncLoad(strPath);
    this.CacheBPObject(strPath, bpObject);
    callback(bpObject);
  }
