...
export default class Misc {
  ...
  static GetCommonText(_strKey: string): string {
    const str = UE.KismetTextLibrary.TextFromStringTable(
      "/Plugins/PuerTSTool/DataTable/ST_Common.ST_Common",
      _strKey,
    );
    return str;
  }
  ...
  static GetErrorCodeText(_strKey: string): string {
    const str = UE.KismetTextLibrary.TextFromStringTable(
      "/Plugins/PuerTSTool/DataTable/ST_ErrorCode.ST_ErrorCode",
      _strKey,
    );
    return str;
  }
  ...
  static GetTSSubsys(): UE.TSSubsystem {
    const pTSSubSys = UE.TSSubsystem.Get(Misc.GetWorld());
    return pTSSubSys;
  }

  //打印日志到屏幕上
  static PrintInScreen(
    _strLog: string,
    duration?: number /* = 2.000000 */,
    textColor?: UE.LinearColor /* = (R=0.000000,G=0.660000,B=1.000000,A=1.000000) */,
  ): void {
  ...
  static IsValid(_object: UE.Object | null | undefined): boolean {
    if (!_object) {
      return false;
    }
  ...
  static AsyncLoad(path: string): Promise<UE.Class> {
    return new Promise<UE.Class>((resolve, reject) => {
      ...
      asyncLoadObj.LoadedCallback.Bind((cls: UE.Class | null) => {
