...
function ApplyAutoMixin(
  _eventName: string,
  eventData: UE.TArray<UE.TSEventData>,
): void {
  ...
  try {
    blueprint.mixin(blueprintClass, info.target, {
      objectTakeByNative: info.objectTakeByNative,
    });
...
function EnsureAutoMixinCallback(): void {
  ...
  tsSubsys.PassTSFunctionAsEvent(
    "ApplyAutoMixin",
    toManualReleaseDelegate(ApplyAutoMixin),
  );
...
export default function mixin(
  ...
  objectTakeByNative = false,
  mode = EMixinMode.Persistent,
) {
  return function <T extends UE.Object>(target: new () => T) {
    ...
    if (mode === EMixinMode.Persistent) {
      ...
      const blueprintClass = blueprint.tojs(ucls);
      return blueprint.mixin(blueprintClass, target, {
        objectTakeByNative,
      }) as any;
    }
  };
